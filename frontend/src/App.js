...
const apiService = {
    ...
    verifyUser: async (email, otp) => {
        try {
            const response = await axios.post(`${API_BASE_URL}/users/verifyUser`, { email, otp });
            return response.data;
        } catch (error) {
            throw error.response?.data || error.message;
        }
        ...
        try {
            if (mode === 'register') {
                if (!showOtpInput) {
                    ...
                    setMessage('OTP sent to your email. Please check and enter the OTP.');
                } else {
                    // Verify OTP
                    await apiService.verifyUser(formData.email, otp);
                    setMessage('Registration successful! You can now login.');
                    setTimeout(() => {
                        onClose();
                        setShowOtpInput(false);
                        setOtp('');
                        setFormData({ name: '', email: '', password: '', isrole: 'patient' });
                    }, 2000);
                }
            } else {
                // Login
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {mode === 'register' && (
                        <select
                            ...
                            onChange={(e) => setFormData({...formData, isrole: e.target.value})}
                            className="w-full bg-slate-700 border border-slate-600 rounded-lg p-3 text-white focus:outline-none focus:ring-2 focus:ring-violet-400"
                        >
                    ...
                    {message && (
                        <div className={`p-3 rounded-lg ${message.includes('successful') ? 'bg-green-600' : 'bg-red-600'} text-white`}>
                            {message}
                        </div>
                    )}
                    ...
                    >
                        {loading ? 'Processing...' : (mode === 'login' ? 'Login' : (showOtpInput ? 'Verify OTP' : 'Register'))}
                    </button>
                </form>
            </div>
        </div>
